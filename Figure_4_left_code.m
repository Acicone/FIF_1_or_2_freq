...
for nn=10%:10
    ...
    for f = df:df:Df
        ...
        for exp_a = -2:da:2
            ...
            for phi = 0:dphi:2*pi
                ...
                [IMF,posF(ii,jj,kk)] = FIF_v4_6(x,opts);
    ...
    save(['Artifical_example_StopCrit_108'])
    %%
    fig=figure;
...
%plot3(-2:da:2,1./sqrt(10.^(-2:da:2)),1*ones(1,101),'k','linewidth',3)
   ...
   ylabel('$f$','Interpreter','latex');
    saveas(fig,'Artifical_example_StopCrit_108', 'png')
    saveas(fig,'Artifical_example_StopCrit_108', 'fig')
    saveas(fig,'Artifical_example_StopCrit_108', 'epsc')
    %%
    fig=figure;
    [X,Y] = meshgrid(-2:da:2,df:df:Df);
    surf(X,Y,mean(c_1(1:end,:,:),3),'edgecolor','none');
...
plot3(-2:da:2,1./(10.^(-2:da:2)).^(1/4),2*ones(1,101),'w','linewidth',3)
    ...
    shading interp
    view(0,90)
    saveas(fig,'Artifical_example_StopCrit_108_top', 'png')
    saveas(fig,'Artifical_example_StopCrit_108_top', 'fig')
    saveas(fig,'Artifical_example_StopCrit_108_top', 'epsc')
     %%
    fig=figure;
    [X,Y] = meshgrid(-2:da:2,df:df:Df);
   ...
   ylabel('$f$','Interpreter','latex');
    set(fig,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    set(gca,'fontsize', 30);
    saveas(fig,'Artifical_example_StopCrit_108_New_Err', 'png')
    saveas(fig,'Artifical_example_StopCrit_108_New_Err', 'fig')
    saveas(fig,'Artifical_example_StopCrit_108_New_Err', 'epsc')
