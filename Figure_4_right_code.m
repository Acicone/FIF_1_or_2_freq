...
for nn=10%:10
    ...
    for f = df:df:Df
        ...
        for exp_a = -2:da:2
            ...
            for phi = 0:dphi:2*pi
                ...
                [IMF,posF(ii,jj,kk)] = FIF_v4_6_1(x,opts);
    ...
    save(['Artifical_example_StopCrit_208'])
    %%
    fig=figure;
...
%plot3(-2:da:2,1./sqrt(10.^(-2:da:2)),1*ones(1,101),'k','linewidth',3)
   ...
   ylabel('$f$','Interpreter','latex');
    saveas(fig,'Artifical_example_StopCrit_208', 'png')
    saveas(fig,'Artifical_example_StopCrit_208', 'fig')
    saveas(fig,'Artifical_example_StopCrit_208', 'epsc')
    %%
    fig=figure;
    [X,Y] = meshgrid(-2:da:2,df:df:Df);
    surf(X,Y,mean(c_1(1:end,:,:),3),'edgecolor','none');
...
plot3(-2:da:2,1./(10.^(-2:da:2)).^(1/3),2*ones(1,101),'g','linewidth',3)
plot3(-2:da:2,1./(10.^(-2:da:2)).^(1/4),2*ones(1,101),'w','linewidth',3)
shading interp
    set(fig,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    set(gca,'fontsize', 30);
    view(0,90)
    saveas(fig,'Artifical_example_StopCrit_208_top', 'png')
    saveas(fig,'Artifical_example_StopCrit_208_top', 'fig')
    saveas(fig,'Artifical_example_StopCrit_208_top', 'epsc')
     %%
    fig=figure;
    [X,Y] = meshgrid(-2:da:2,df:df:Df);
   ...
   ylabel('$f$','Interpreter','latex');
    set(fig,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    set(gca,'fontsize', 30);
    saveas(fig,'Artifical_example_StopCrit_208_New_Err', 'png')
    saveas(fig,'Artifical_example_StopCrit_208_New_Err', 'fig')
    saveas(fig,'Artifical_example_StopCrit_208_New_Err', 'epsc')
