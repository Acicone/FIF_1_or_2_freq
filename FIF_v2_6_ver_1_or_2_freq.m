function [IMF,posF] = FIF_v2_6_ver_1_or_2_freq(f,options,M)


%
%  function IMF = FIF_v2_6(f,options)
%
...
if nargin < 1,  help FIF_v2_6; return; end
...
while countIMFs < options.IF.NIMFs && k_pp>=options.IF.ExtPoints && toc<options.maxTime
    ...
    if isempty(M) || length(M)<countIMFs
        ...
        if isa(options.IF.alpha,'char')
            ...
            m = 2*round(options.IF.Xi*prctile(diffMaxmins_pp,options.IF.alpha));
%             if not(prctile(diffMaxmins_pp,options.IF.alpha)==200)
...
%     end
    ...
    % we look for the closest minimum to position 201 in ifftA
    
    posF=201;%find((diff(ifftA)>0)~=0,1,'first');
    ...
    while SD>options.IF.delta && inStepN < options.IF.MaxInner
        ...
        if options.plots>=1  && rem(inStepN,2)==0   
            ...
            plot(ifft(fft_h_new),'linewidth',2)
            figure(figRem)            
            title(['Remainder after IMF ' num2str(countIMFs) ' step # ' num2str(inStepN) ])
            plot(f-ifft(fft_h_new),'linewidth',2)
...
%         set(figMask,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_FIF_v2_6.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_FIF_v2_6.mat'],'IMF','logM','-v7.3');
end
